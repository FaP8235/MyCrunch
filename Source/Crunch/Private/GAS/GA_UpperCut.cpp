...
const FGenericDamageEffectDef* UGA_UpperCut::GetDamageEffecctDefForCurrentCombo() const
{
...
void UGA_UpperCut::StartLaunching(FGameplayEventData EventData)
{
	...
	UAbilityTask_WaitGameplayEvent* WaitComboChangeEvent = UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(this, UGA_Combo::GetComboChangeEventTag(), nullptr, false, false);
...
void UGA_UpperCut::HandleComboChangeEvent(FGameplayEventData EventData)
{
	...
	if (EventTag == UGA_Combo::GetComboChangeEventEndTag())
	{
		NextComboName = NAME_None;
...
void UGA_UpperCut::HandleComboDamageEvent(FGameplayEventData EventData)
{
	if (K2_HasAuthority())
	{
		...
		const FGenericDamageEffectDef* EffectDef = GetDamageEffecctDefForCurrentCombo();
		if (!EffectDef)
		{
			return;
		}
